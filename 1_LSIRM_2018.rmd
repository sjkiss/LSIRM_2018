...
#### Assignment 1
**Combine `age`, `gender`   and `lucky`  into one matrix called `matrix1   with 3 columns and n rows using the `matrix()`   command, the `ncol=` and/or the `nrow=`  arguments.** 

[Solution](#assignment1)

```{r assignment-1-solution-hidden, results='hide', echo=F}
...
#### Assignment 2
**Get your own age from `age`. Get your own gender from `gender`. The trick is just remembering where in order of ages and genders yours were stored in.**   
...
#### Assignment 5
** Make a data frame called `df` with the variables `age`, `gender` and `lucky` from the vectors we defined above `age`, `gender` and `lucky`.

```{r make-data-frame, results='hide'}
...
Write a function that takes one argument and then prints it out. Hint: we printed a result early in the class. Save the function as `myfunction`. Try to get it to work. 

[Solution](#assignment6)

# Solutions

#### Assignment 1 Solution {#assignment1}
```{r assignment-1-solution, echo=TRUE}
matrix(c(age, gender, lucky), nrow=3, ncol=3)

```
#### Assignment 2 Solution {assignment2}

```{r assignment-2-solution, eval=F}
...
```{r assignment-5-solution, eval=F}
df<-data.frame(age=age, gender=gender, lucky=lucky)
```

#### Assignment 6 Solution {assignment6}
```{r assignment-6-solution, eval=F}
#Defie the function
