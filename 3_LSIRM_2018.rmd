...
Part of the problem stems from the fact that the value labels are not in a meaningful order, i.e. `1=Better`, but `2=Worse`, but `3=Stayed the same`.That is not our fault, but it is our problem. It would be more intuitive to have the levels go from one extreme to the other.  

We coudl attack this from a number of ways. The easiest way, though would be as follows.

#### Assignment 3
> 1.Recode the variables `pers_ret` and `pers_fdpol` into two new variables. Save them as `personal` and `federal`. This relies on turning the `labelled` variable into a `number` with the function `as.numeric()`.  The `Recode()` function **does not work** with labelled variables. Set the levels to go in an order from `Worse` to `Better`.
...
#### Assignment 5
> 1. In the plot for Assignment 4 that we made, there were missing values that were reported. Eliminate the missing values using `filter()` and `is.na()`. 
...
#### Assignment 6
>1. Recode `p_votechce` to include only Conservative, NDP, Liberal, BQ voters . Store it in `vote`. Remember to use `vals` and `as.numeric`. 
...
There are tons of tutorials out there.  Here are some links to some of the most usefl.
