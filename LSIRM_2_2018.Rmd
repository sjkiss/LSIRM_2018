...
It looks like for every variable, except for `income_num` and `p_selfplace`, we can Recode values 998 to 1000 to be missing. But in addition, some variables respondents also picked `don't know` or `Refuse`. Some might try to make use of those data, but we'll just dump them for the moment. 

The easiest function to recode variables is the `Recode` command in the `car` library. 
So, for the income inquality variable, we could safely recode any number larger than 3 to be missing. Or we could specify each individual number. Both of the following commands are equivalent. 
...
Notice now that the maximum value is 2. We've set missing values to be `NA`. 

We just dumped that recode result in a quick temporary variable called `test`. 

What we really want is to save it in a new variable caled `inequal`. When you execute this command, watch what happens to the environment in the top right corner. 
...
#### Assignment 4
> 1. Recode `each` of the other variables such that missing values are converted to NA. You will have to go back and forth to `vals` to remind yourself of what the missing values are. 
...
There is one last wonky thing we need to deal with. `income_num` has a minimum value of `-99000` and `p-selfplace` has a maximum value of `1000`.  The reason they did not show up when we looked through `vals` is that they are not read in as categorical variables from the SPSS file; they were read in as numbers, without any value labels attached to them. 
...
#### Assignment 5
> 1. Recode the `p_selfplace` variable so that 1000 is NA; This time, store the variable as `ideology`.

```{r assignment-solution-5}
#Recode p_selfplace
...
We can do gender as follows.  First, check the variables. 
```{r check-variables}
#Check sex
ces$sex_r
...
This will not work that well. Let's recode the variable to a factor. 

```{r Recode-gender-factor}
...
#### Assignment 9
> 1. Recode `education` into a dichotomous variable, called `degree`. 
> 2. Produce another scatterplot income and ideology but group the points by education using color to differentiate
